...
document.querySelector('#dismiss-notification')?.addEventListener('click', () => {
  document.querySelector('#incoming-notification')?.classList.add('is-dismissed');
});

document.querySelector('#open-answer')?.addEventListener('click', () => {
  const answer = document.querySelector('#answer-message');
  answer?.scrollIntoView({ behavior: 'smooth', block: 'center' });
  answer?.focus({ preventScroll: true });
  answer?.classList.add('is-highlighted');
  setTimeout(() => answer?.classList.remove('is-highlighted'), 1600);
});
