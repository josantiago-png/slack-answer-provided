...
Next Steps: Let's spin up a quick dev environment to test the circuit breaker thresholds. Can you share the specific Envoy/Mesh YAML file you are looking at? I’ll gladly jump in and co-author the updated configuration with you.`,T=[{id:"service-mesh-timeout-answer",time:"10:32 AM",blocks:[{type:"section",style:"quote",text:{type:"mrkdwn",text:`${w}

*${p}*

${M}`}},{type:"section",style:"answer",text:{type:"mrkdwn",text:`<@drew> answered: ${m}`}}]},{id:"cloud-infra-alert-pipeline-correct",time:"10:44 AM",blocks:[{type:"section",style:"quote",text:{type:"mrkdwn",text:`${S}

*${x}*

${I}`}},{type:"status",text:"user marked this answer as correct"}]}],n=e=>e.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;"),g=e=>n(e).replace(/\*([^*]+)\*/g,"<strong>$1</strong>").replace(/&lt;@([^>]+)&gt;/g,'<span class="mention">@$1</span>'),u=e=>e.type==="mrkdwn"?g(e.text):n(e.text),c=(e="")=>`
...
`,t=(e,a="ui-icon")=>`
  ...
  `,C=e=>t("dms","sidebar-icon"),D=e=>e.type==="header"?`<div class="bk-block bk-header">${u(e.text)}</div>`:e.type==="section"?`<div class="bk-block bk-section" data-style="${e.style||"body"}">${u(e.text)}</div>`:e.type==="context"?`<div class="bk-block bk-context">${e.elements.map(a=>`<span>${u(a)}</span>`).join("")}</div>`:e.type==="status"?`
...
`,_=()=>`
  <div class="channel-date-line"><span>Today</span></div>
  ${P()}
`,k=e=>e==="infra"?_():e==="general"?z():B(e),j=()=>`
    ...
    <aside class="slack-sidebar" aria-label="Acme Demo navigation">
      ...
      <nav class="sidebar-nav" aria-label="Slack sections">
        ...
        <div class="sidebar-group">
          ...
          ${q.map(e=>`<a href="#" class="sidebar-link channel-link ${e==="general"?"is-active":""}" data-channel="${e}" ${e==="general"?'aria-current="page"':""}># ${e}</a>`).join("")}
        </div>
      </nav>
    </aside>
