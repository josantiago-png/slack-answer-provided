(function(){const a=document.createElement("link").relList;if(a&&a.supports&&a.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))d(n);new MutationObserver(n=>{for(const o of n)if(o.type==="childList")for(const c of o.addedNodes)c.tagName==="LINK"&&c.rel==="modulepreload"&&d(c)}).observe(document,{childList:!0,subtree:!0});function r(n){const o={};return n.integrity&&(o.integrity=n.integrity),n.referrerPolicy&&(o.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?o.credentials="include":n.crossOrigin==="anonymous"?o.credentials="omit":o.credentials="same-origin",o}function d(n){if(n.ep)return;n.ep=!0;const o=r(n);fetch(n.href,o)}})();const S=document.querySelector("#app"),f="I am trying to figure out how to resolve this issue, but I cannot figure out if this is the right approach. Can you please review it and confirm?",h="Should we resolve the service mesh timeout issue by increasing retries?",w="Increasing retries can help with short transient failures, but it should not be the only fix. The safer approach is to validate timeout budgets, add circuit breaker limits, and only raise retries after confirming the upstream service can absorb the extra load.",x="I need a review on this rollout plan before I share it with the incident response group. Does this look like the right sequence?",I="What is the recommended plan for migrating the Cloud Infra alert pipeline without missing critical pages?",q="Use a staged migration: freeze alert rule edits before the window, export the current PagerDuty routing rules and service ownership map, run the new pipeline in shadow mode, compare events for at least two business days, and only cut over after confirming parity with the incident response leads.",g=[{title:"Cloud Infra service mesh retry guidance",meta:"Google Docs"},{title:"#infra thread: service mesh timeout budget",meta:"Slack"},{title:"Envoy circuit breaker rollout notes",meta:"Google Docs"},{title:"Platform reliability review: retry storms",meta:"Slack"},{title:"Cloud Infra ownership map",meta:"Stack Internal"},{title:"Incident response runbook",meta:"Google Docs"},{title:"#platform conversation: upstream saturation",meta:"Slack"},{title:"Service mesh production defaults",meta:"Stack Internal"},{title:"Retry policy migration checklist",meta:"Google Docs"},{title:"Cloud Infra weekly capacity notes",meta:"Slack"}],T=g.slice(0,6),C=["announcements","general","engineering","backend","frontend","platform","infra","security","product","design","data-engineering","ml-ai","sales","marketing","hr","watercooler"],L=[{author:"Rina Patel",time:"9:42 AM",avatar:"RP",tone:"blue",text:"Morning. I moved the customer escalation notes into the shared launch doc and tagged the support leads."},{author:"Alex Chen",time:"9:48 AM",avatar:"AC",tone:"green",text:"Thanks. I am going to use the same doc for the handoff checklist so everyone has one place to look."},{author:"Maya Torres",time:"10:03 AM",avatar:"MT",tone:"purple",text:"Can someone from platform confirm whether the deploy freeze starts at noon or after the metrics review?"},{author:"Drew Meacham",time:"10:08 AM",avatar:"DM",tone:"orange",text:"After the metrics review. We still have one config validation to complete before the freeze window starts."}],M=`To answer your question directly: No, we shouldn't just increase retries as a standalone fix. While cranking up retries is a tempting quick-fix for transient blips, doing it in isolation can accidentally trigger a "retry storm"—essentially DDoS'ing our own upstream services when they are already struggling.
...
Next Steps: Let's spin up a quick dev environment to test the circuit breaker thresholds. Can you share the specific Envoy/Mesh YAML file you are looking at? I’ll gladly jump in and co-author the updated configuration with you.`,D=[{id:"service-mesh-timeout-answer",time:"10:32 AM",blocks:[{type:"section",style:"body",text:{type:"mrkdwn",text:`From <@Alex Chen>: ${f}`}},{type:"section",style:"question",text:{type:"mrkdwn",text:`*${h}*`}},{type:"section",style:"body",text:{type:"mrkdwn",text:w}},{type:"sources",sourceList:T},{type:"answered",text:"answered by drew"},{type:"replySummary",channel:"infra",label:"1 reply",time:"Today at 9:37 AM"}]},{id:"cloud-infra-alert-pipeline-correct",time:"10:44 AM",blocks:[{type:"section",style:"body",text:{type:"mrkdwn",text:`From <@Marco Lin>: ${x}`}},{type:"section",style:"question",text:{type:"mrkdwn",text:`*${I}*`}},{type:"section",style:"body",text:{type:"mrkdwn",text:q}},{type:"status",text:"user marked this answer as correct"}]}],s=e=>e.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;"),y=e=>s(e).replace(/\*([^*]+)\*/g,"<strong>$1</strong>").replace(/&lt;@([^>]+)&gt;/g,'<span class="mention">@$1</span>'),v=e=>e.type==="mrkdwn"?y(e.text):s(e.text),i=(e="")=>`
...
`,t=(e,a="ui-icon")=>`
  ...
  `,E=e=>t("dms","sidebar-icon"),H=e=>e.type==="header"?`<div class="bk-block bk-header">${v(e.text)}</div>`:e.type==="section"?`<div class="bk-block bk-section" data-style="${e.style||"body"}">${v(e.text)}</div>`:e.type==="context"?`<div class="bk-block bk-context">${e.elements.map(a=>`<span>${v(a)}</span>`).join("")}</div>`:e.type==="sources"?k(e.sourceList||g):e.type==="answered"?`<div class="bk-block bk-answered">${s(e.text)}</div>`:e.type==="replySummary"?`
      <button class="thread-summary thread-summary--answer" type="button" data-open-answer-thread="${s(e.channel)}">
...
`,p=e=>`<span class="format-control format-control--${e}">${{link:`
...
`,j=()=>`
  <div class="channel-date-line"><span>Today</span></div>
  ${z()}
`,A=e=>e==="infra"?j():e==="general"?B():_(e),N=()=>`
    ...
    <aside class="slack-sidebar" aria-label="Acme Demo navigation">
      ...
      <nav class="sidebar-nav" aria-label="Slack sections">
        ...
        <div class="sidebar-group">
          ...
          ${C.map(e=>`<a href="#" class="sidebar-link channel-link ${e==="general"?"is-active":""}" data-channel="${e}" ${e==="general"?'aria-current="page"':""}># ${e}</a>`).join("")}
        </div>
      </nav>
    </aside>
...
`;const l=e=>{document.querySelectorAll("[data-surface]").forEach(a=>{a.classList.toggle("is-hidden",a.dataset.surface!==e)}),document.querySelectorAll("[data-nav-target]").forEach(a=>{const r=a.dataset.navTarget===e;a.classList.toggle("is-active",r),r?a.setAttribute("aria-current","page"):a.removeAttribute("aria-current")}),e!=="channel"&&document.querySelectorAll("[data-channel]").forEach(a=>{a.classList.remove("is-active"),a.removeAttribute("aria-current")})},u=(e,{openThread:a=!1}={})=>{const r=document.querySelector(".channel-workspace"),d=document.querySelector("[data-channel-heading]"),n=document.querySelector("[data-channel-body]"),o=document.querySelector(".channel-view [data-composer-placeholder]");r.dataset.currentChannel=e,d.textContent=`#${e}`,n.innerHTML=A(e),o.textContent=`Message #${e}`,document.querySelectorAll("[data-channel]").forEach(c=>{const b=c.dataset.channel===e;c.classList.toggle("is-active",b),b?c.setAttribute("aria-current","page"):c.removeAttribute("aria-current")}),r.classList.toggle("is-thread-closed",e!=="infra"||!a)};document.querySelectorAll("[data-nav-target]").forEach(e=>{e.addEventListener("click",a=>{a.preventDefault(),l(e.dataset.navTarget)})});document.querySelectorAll("[data-channel]").forEach(e=>{e.addEventListener("click",a=>{a.preventDefault(),l("channel"),u(e.dataset.channel)})});document.querySelectorAll("[data-thread-shortcut]").forEach(e=>{e.addEventListener("click",a=>{a.preventDefault(),l("channel"),u(e.dataset.threadShortcut,{openThread:!0}),document.querySelector(".channel-message")?.scrollIntoView({block:"center",behavior:"smooth"})})});document.querySelectorAll("[data-open-channel-thread]").forEach(e=>{e.addEventListener("click",()=>{l("channel"),u(e.dataset.openChannelThread,{openThread:!0})})});document.querySelectorAll(".thread-close").forEach(e=>{e.addEventListener("click",()=>{document.querySelector(".channel-workspace")?.classList.add("is-thread-closed")})});document.addEventListener("click",e=>{e.target.closest("[data-open-thread]")&&document.querySelector(".channel-workspace")?.classList.remove("is-thread-closed");const a=e.target.closest("[data-open-answer-thread]");a&&(l("channel"),u(a.dataset.openAnswerThread,{openThread:!0}))});document.querySelectorAll(".tab-button").forEach(e=>{e.addEventListener("click",()=>{const a=e.dataset.tab;document.querySelectorAll(".tab-button").forEach(r=>{r.classList.toggle("is-active",r===e)}),document.querySelectorAll(".tab-panel").forEach(r=>{r.classList.toggle("is-active",r.dataset.panel===a)})})});
